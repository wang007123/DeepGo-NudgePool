...
describe("GP running liquidation", function() {
    before(async function() {
    loger("===================================================================================");
    loger("test time: " + new Date().toString());
    loger("-------------- Task Description -------------");
    loger("test name: GP running liquidation");
    ...
    this.gp1DepositTokenAmount = getBigNumber(5000);
    this.gp2DepositTokenAmount = getBigNumber(1000);
    this.lp1DepositTokenAmount = getBigNumber(10000);
    this.lp2DepositTokenAmount = getBigNumber(20000);
  ...
  it("LP deposit running", async function() {
    ...
    const raiseLPAmount = await this.NudgePoolStatus.getCurRaiseLPAmount(
      this.token1,
      this.token2
    );
    loger("Raise LP amount: " + Math.round(raiseLPAmount/this.ether));
  });
  ...
  it("check GP liquidation", async function() {
    const stage = await this.NudgePoolStatus.getPoolStage(this.token1, this.token2);
    // Return while not at running stage
    if (stage != 4) {
      return
    ...
    const checkGPLiquidationTx = await this.NudgePool.checkGPLiquidation(
      this.token1,
      this.token2
    );
    await checkGPLiquidationTx.wait();
